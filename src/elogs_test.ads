...
private
   ...
   --  Latest_Message returns the most recently stored log. Note that this test
   --  is quite fragile as it depends on the loop and a log and exception above
   --  and expects that the index is back to the beginning of the Log_Store.
